...
// SPL Device Ready Check Function
uint8_t SSD1306_IsDeviceReady(void) {
    ...
    // 2. Send slave address for write (or read, doesn't matter for device ready check)
    ...
    return 0; // Device is ready (ACKed address)
}
...
void SSD1306_I2C_Write(uint8_t address, uint8_t reg, uint8_t data) {
    // Use SPL for single byte write
    I2C_WriteByteToSlave(address, reg, data);
}
*/
void SSD1306_I2C_Write(uint8_t address, uint8_t reg, uint8_t data) {
    __IO uint32_t timeout = 1000;

    // B?t d?u truy?n d? li?u
    I2C_GenerateSTART(I2C1, ENABLE);
    while (!I2C_CheckEvent(I2C1, I2C_EVENT_MASTER_MODE_SELECT)) {
        if ((timeout--) == 0) return;
    }

    // G?i d?a ch? slave (OLED)
    I2C_Send7bitAddress(I2C1, address, I2C_Direction_Transmitter);
    timeout = 1000;
    while (!I2C_CheckEvent(I2C1, I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED)) {
        if ((timeout--) == 0) return;
